# YouTube OS — Canonical Episode State Architecture
...
- **Tests:** 29/29 passing
- **Responsibility:** Pure function mapping Snapshot → CardProps
...
- **Adapter:** Pure function mapping canonical → snapshot

Example:
- Canonical field: `workflowState: "QA"`
- Snapshot field: `workflowStateLabel: "Quality Assurance"` (optional UI hint)
- UI rendering: Adapter maps state → variant → card appearance
...
**Identity:**
- `episodeId` — Canonical, persistent ✓
- `episodeNumber` — Canonical, persistent ✓
- `channelName` — Canonical, persistent ✓
- `title` — Canonical, persistent ✓

**Workflow:**
- `workflowState` — Canonical, persistent ✓
- `workflowStateLabel` — Derived/UI hint, may be omitted ⚠
- `reviewStatus` — Canonical, persistent ✓
- `nextAuthorizedAction` — Derived from state, computed ⚠

**State:**
- `blockers` — Canonical, persistent ✓
- `lastDecision` — Canonical, persistent ✓

**Publication:**
- `publication.youtubeVideoId` — Canonical, persistent ✓
- `publication.publishedAt` — Canonical, persistent ✓

**System:**
- `source` (version, fetchedAt) — Metadata, maybe derival ⚠
- `unavailableReason` — System error indicator, should be persistent ✓

### 3.2 Fields to Add to Canonical Schema

- `schemaVersion` — Track schema evolution
- `stateVersion` — Optimistic locking for concurrency
- `createdAt` — Episode creation timestamp
- `updatedAt` — Last state change timestamp
- `workflowStateVersion` — What version of workflow model is this?
...
**Indexes:**
- Primary: `episodeId`
- Secondary: `(channelId, episodeNumber)` — for catalog queries
- Secondary: `workflowState` — for workflow reporting
- Secondary: `(publishedAt DESC)` — for published history
...
- Secondary: `(episodeId, createdAt DESC)` — retrieve episode history
- Secondary: `eventType` — query by event type
- Secondary: `idempotencyKey` — idempotency checks
...
**Example Blocker IDs:**
- `thumbnail-direction` — Designer sign-off needed
- `fact-check-failed` — Fact-checking found errors
- `render-artifact` — Visual glitch in render
- `audio-mismatch` — Narration duration mismatch
...
Actor 1 attempts same change:
  UPDATE episodes
    SET workflowState = 'QA', stateVersion = 18
    WHERE episodeId = 'episode-014' AND stateVersion = 17
  → Fails (0 rows updated, Actor 2 already won)
  → Client sees conflict
  → Retries with fresh read of stateVersion = 18
```
...
4. If 0 rows updated: **Conflict** → refresh and retry
5. If 1 row updated: **Success** → stateVersion incremented
...
First attempt:
  ...
  → Creates EpisodeEvent with idempotencyKey
  → Returns success

n8n retry (same execution):
  POST /api/episodes/episode-014/transition
    { toState: "QA", idempotencyKey: "n8n-exec-xyz" }
  → Finds existing EpisodeEvent with same idempotencyKey
  → Returns "already done" (no duplicate event)
...
Event: REVIEW_REQUESTED
  → Episode.reviewStatus = "in-progress"
  → YouTube OS UI shows "awaiting decision"
...
Event: DECISION_RECORDED
  → Episode.latestDecision = { outcome: "pass", decidedBy: "alice@example.com", ... }
  → Episode.reviewStatus = "completed"

n8n polls:
  GET /api/episodes/episode-014
  → sees latestDecision is now present
  → continues workflow (publishes or whatever next step is)
```
...
|---|---|---|---|---|
| Durability | ✅ HA, backups | ⚠ File-based | ✅ Managed | ❌ Ephemeral |
| Concurrent writes | ✅ Optimistic locking | ❌ Locks | ✅ Built-in | ❌ Not designed for it |
| Queryability | ✅ SQL, complex | ✅ SQL, limited | ✅ Document queries | ❌ Limited |
| Cost | ✅ $0–30/mo | ✅ Free | ❌ $1–100/mo (metered) | ❌ Coupled to n8n |
| Vercel integration | ✅ Native Vercel Postgres | ⚠ Via file storage | ⚠ Via Firebase | ❌ None |
| n8n integration | ✅ HTTP API + webhooks | ⚠ File polling | ✅ API | ❌ Internal |
| Audit trail | ✅ Event log table | ❌ No built-in | ⚠ Timestamp fields | ❌ Not designed |
| Migration path | ✅ Scalable to Postgres pro | ⚠ Complex | ❌ Vendor lock-in | ❌ Lock-in |
...
- `users` — If humans edit state directly via UI
- `audit_log` — If compliance requires immutable audit trail
- `webhooks` — If external systems need state change notifications
...
```
n8n request:
  → n8n_webhook_secret in Authorization header
  → Verify against env var
  → Execute command
  → Log actor = "n8n"

YouTube OS UI request:
  → Runs on server (no credentials needed)
  → Actor = logged-in user or "system"
```
...
```
Database → YouTube Provider → Snapshot → Adapter → Component → UI
```
...
- [ ] Build data migration: `fixtures → database`
...
└────────┬────────────────────┘
         ...
         │ 2. Check idempotency
         │ 3. Update episode (stateVersion-protected)
         │ 4. Append event (audit trail)
         ↓
┌──────────────────────────────┐
│  Vercel Postgres             │
├──────────────────────────────┤
│ episodes (current state)     │
│ episode_events (audit log)   │
└────────┬─────────────────────┘
         │
         │ SELECT episodeId, workflowState, blockers, ...
         ↓
┌─────────────────────────────┐
│  YouTube Provider           │
│  (Server-side wrapper)      │
...
└────────┬────────────────────┘
         │
         │ Map Episode → EpisodeStateSnapshot
         ↓
┌─────────────────────────────┐
│  EpisodeStateSnapshot       │
...
└────────┬────────────────────┘
         │
         │ (Adapter) Map snapshot → card props
         ↓
┌─────────────────────────────┐
│  EpisodeStateCardProps      │
└────────┬────────────────────┘
...
3. Write data migration script (fixtures → database)
...
This design ensures:
- ✅ State durability independent of n8n
- ✅ Concurrent writes don't lose data
- ✅ Audit trail is immutable
- ✅ Idempotent retries work correctly
- ✅ Human decisions are durable
- ✅ UI doesn't know about n8n
- ✅ UI doesn't expose database
- ✅ Blocker lifecycle is tracked
