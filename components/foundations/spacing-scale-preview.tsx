...
export function SpacingScalePreview({ tokens }: { tokens: SpacingToken[] }) {
  return (
    <div className="overflow-hidden rounded-2xl border border-stone-300 bg-[#fbfaf6]">
      <div className="grid grid-cols-[0.8fr_0.8fr_1.4fr_1fr] gap-0 border-b border-stone-300 bg-stone-100/70 px-4 py-3 text-[10px] font-mono font-semibold uppercase tracking-[0.18em] text-neutral-500">
        <span>Role</span>
        <span>Value</span>
        <span>Usage</span>
        <span>Limits</span>
      ...
      <div className="divide-y divide-stone-200">
        {tokens.map((token) => (
          <div key={token.role} className="grid grid-cols-[0.8fr_0.8fr_1.4fr_1fr] gap-0 px-4 py-4 text-sm">
