...
export type ImageRippleRenderStatus = "initializing" | "active" | "fallback"
...
export interface ImageRippleEffectProps
  ...
  onRenderStatusChange?: (status: ImageRippleRenderStatus) => void;
  diagnosticMode?: "final" | "source" | "displacement";
  maxWaveCount?: number;
  mobileWaveCount?: number;
  children?: React.ReactNode;
}


function seededUnit(seed: number, index: number) {
  const value = Math.sin(seed * 1000 + index * 12.9898) * 43758.5453;
  return value - Math.floor(value);
}

function getCapturePoint(progress: number, width: number, height: number) {
  const clamped = Math.min(1, Math.max(0, progress));
  const angle = clamped * Math.PI * 1.2;
  return {
    x: width * (0.18 + clamped * 0.64),
    y: height * (0.52 + Math.sin(angle) * 0.18),
  };
}
...
function RippleFallback({
  ...
  return <WebGLFallback className={className} message={message} />;
}
...
function RippleScene({
  ...
  React.useEffect(() => {
    // Texture configuration is required by Three.js after useTexture resolves.
  ...
  React.useEffect(() => {
    ...
    for (let i = 0; i < waveCount; i += 1) {
      ...
      mesh.visible = false;
      mesh.rotation.z = seededUnit(seed, i) * Math.PI * 2;
  ...
  }, [brushTexture, rippleScene, seed, waveCount, waveSize]);
  ...
  React.useEffect(() => {
    ...
    images.forEach((item, index) => {
      ...
      mesh.position.x = (item.x ?? (index - (images.length - 1) / 2) * 0.25) * width;
      mesh.position.y = (item.y ?? 0) * height;
      mesh.position.z = 1;
      mesh.scale.x = width * (item.widthScale ?? 0.72);
      mesh.scale.y = height * (item.heightScale ?? 0.72);
      group.add(mesh);
    });
  ...
  }, [height, imageCamera, imageScene, imageTextures, images, width]);

  useFrame(() => {
    const capturePoint = captureMode ? getCapturePoint(progress, width, height) : null;
    const sourceX = capturePoint?.x ?? pointerRef.current.x;
    const sourceY = capturePoint?.y ?? pointerRef.current.y;
    const x = sourceX - width / 2;
    const y = -sourceY + height / 2;
    ...
    if (moved && waveMeshesRef.current.length > 0) {
      ...
      if (mesh) {
        ...
        mesh.rotation.z = seededUnit(seed, waveIndex + currentWaveRef.current) * Math.PI * 2;
    ...
    uniforms.uTexture.value = diagnosticMode === "displacement" ? fboBase.texture : fboTexture.texture;
    uniforms.uDisplacement.value = diagnosticMode === "source" ? fboTexture.texture : fboBase.texture;
    uniforms.uStrength.value = diagnosticMode === "final" ? distortionStrength : 0;
    uniforms.winResolution.value
      .set(width, height)
...
export function ImageRippleEffect({
  ...
  reduceMotion = false,
  forceFallback = false,
  fallbackMessage = "Interactive ripple rendering is unavailable.",
  ...
  const effectiveWaveCount = Math.max(1, Math.min(waveCount, maxWaveCount, isMobile ? mobileWaveCount : maxWaveCount));
  ...
  }, [height, pointerX, pointerY, width]);

  const staticImage = images[0]?.src ?? DEFAULT_IMAGE_URLS[0];
  const shouldRenderFallback = reduceMotion || forceFallback;

  React.useEffect(() => {
    hasReportedFirstFrameRef.current = false;
    onRenderStatusChange?.(shouldRenderFallback ? "fallback" : "initializing");
  }, [diagnosticMode, onRenderStatusChange, progress, seed, shouldRenderFallback]);

  const handleFirstFrame = React.useCallback(() => {
    if (!hasReportedFirstFrameRef.current) {
      hasReportedFirstFrameRef.current = true;
      onRenderStatusChange?.("active");
    }
  }, [onRenderStatusChange]);
  ...
  if (shouldRenderFallback) {
    return (
      <div
        ref={containerRef}
        className={cn("relative h-[560px] w-full overflow-hidden bg-zinc-950 text-white", className)}
        {...props}
      >
        <div role="img" aria-label={fallbackMessage} className="absolute inset-0 bg-cover bg-center" style={{ backgroundImage: `url(${staticImage})` }} />
        <div className="absolute inset-0 bg-black/35" />
        <RippleFallback className="absolute inset-0 h-full w-full bg-transparent" message={fallbackMessage} onVisible={() => onRenderStatusChange?.("fallback")} />
  ...
  return (
    ...
    >
      {width > 0 && height > 0 && (
        <WebGLErrorBoundary fallback={<RippleFallback className="absolute inset-0 h-full w-full" message={fallbackMessage} onVisible={() => onRenderStatusChange?.("fallback")} />}>
          <Canvas dpr={Math.min(pixelRatio, 1.75)} frameloop="always">
            <OrthographicCamera
              makeDefault
              args={[
