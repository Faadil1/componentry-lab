# Episode State Card — Promotion Review
...
**PROMOTE** — Episode State Card v1.0.0 satisfies all five promotion readiness dimensions and clears all four verification gates.
...
- Motion respects system `prefers-reduced-motion` via `useReducedMotion()` — no user-facing override needed

### 2. API Readiness

- API stabilized at 2026-08-06T11:00:00Z — `api_stabilized: true`
...
- `HumanReviewStatus`: `"not-required" | "required" | "completed"` — intentionally excludes `"unavailable"`
...
| Gate | Result |
|------|--------|
| `npx tsc --noEmit` | EXIT 0 — 0 unused `@ts-expect-error` directives |
...
- Component is read-only display — no side effects, no state mutations
...
- Exports 12 intentional public symbols — no internal helpers leak
...
**Next phase: INTEGRATION_READY** — Component is promoted into the live registry catalog and available for integration into production surfaces.
