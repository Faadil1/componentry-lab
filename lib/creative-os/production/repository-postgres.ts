...
export async function saveRoutePostgres(route: ProductionRoute): Promise<{ status: "CREATED" | "ALREADY_REGISTERED" | "INSUFFICIENT_AUTHORITY" | "INVALID_INPUT" | "REPOSITORY_CORRUPT" | "PERSISTENCE_FAILED"; route?: ProductionRoute; existingRoute?: ProductionRoute; error?: string }> {
  ...
  const existing = await sql`
    SELECT payload
    FROM componentry_routes
    WHERE route_identity = ${routeIdentity(route)}
  `
  ...
  await sql`
    ...
    VALUES (${route.routeId}, ${route.projectId}, ${route.planFingerprint}, ${routeIdentity(route)}, ${JSON.stringify(route)}::jsonb, ${now}, ${now})
  `
