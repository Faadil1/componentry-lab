...
function canCreateRoute(authorityContext: AuthorityContext): boolean {
  return (
    ...
    authorityContext.grantedScope.includes("production:entry")
  )
}

function routeFingerprint(input: EnterProductionInput): string {
  return [
    input.project.id,
    input.plan.planFingerprint,
    input.plan.capabilityId,
    input.plan.resourceId ?? "none",
    input.plan.requestedArtifact ?? "none",
    input.approval.approvalFingerprint,
    input.approval.approvalState,
    input.authorityContext.authorityLevel,
    input.authorityContext.approvalRequirement,
    input.authorityContext.reversibility,
    input.authorityContext.grantedScope.join(","),
  ].join("|")
}
...
export function createRoute(input: EnterProductionInput): EnterProductionResult {
  if (!canCreateRoute(input.authorityContext)) {
    return { status: "INSUFFICIENT_AUTHORITY", error: "Local reversible authority with explicit production entry approval is required." }
  }

  if (!input.project || !input.plan || !input.approval) {
    return { status: "INVALID_INPUT", error: "Project, plan, and approval are required." }
  }
  ...
  const expectedFingerprint = routeFingerprint(input)
  const nextRoute = resolveProductionRoute(input.plan, input.project.id, false)
  const route: ProductionRoute = {
    ...nextRoute,
    routeId: `route_${expectedFingerprint.replace(/[^a-zA-Z0-9]+/g, "_")}`,
  ...
  if (input.approval.projectId !== input.project.id || input.approval.planFingerprint !== input.plan.planFingerprint || input.approval.resourceId !== input.plan.resourceId || input.approval.capabilityId !== input.plan.capabilityId) {
    return { status: "INVALID_INPUT", error: "Approval must bind to the exact project, plan, capability, and resource." }
  }
  ...
  try {
    writeRepositoryAtomic({ routes: [...existingRoutes, route] })
  } catch (error) {
    try {
