...
describe("Episode State Card — API Stabilization", () => {
  ...
  test("20. Public API version is 1.0.0 and registry reflects PROMOTED state", () => {
    ...
    // Promoted state
    assert.strictEqual(content.includes("status: PROMOTED"), true)
    // Stabilization complete, approved, and promoted
    assert.strictEqual(content.includes("api_stabilized: true"), true)
    assert.strictEqual(content.includes("approved: true"), true)
    assert.strictEqual(content.includes("promoted: true"), true)
